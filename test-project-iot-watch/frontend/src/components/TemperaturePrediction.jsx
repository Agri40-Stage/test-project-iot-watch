...
const TemperaturePrediction = () => {
  ...
  const fetchPredictions = async () => {
    try {
      setLoading(true);
      console.log(`Fetching predictions from: /api/predict?day=${predictionDay}`);
      setDebugInfo(`Attempting to fetch from: /api/predict?day=${predictionDay}`);
      
      // Use relative URL with Vite proxy
      const response = await fetch(`/api/predict?day=${predictionDay}`);
      ...
      if (err.message.includes("Not enough historical data")) {
        try {
          ...
          const mockDataResponse = await fetch(`/api/insert-mock-data`, {
            method: 'POST'
          });
