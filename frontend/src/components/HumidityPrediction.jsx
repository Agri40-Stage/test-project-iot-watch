...
const HumidityPrediction = () => {
  ...
  const fetchPredictions = async () => {
    try {
      setLoading(true);
      console.log(`Fetching humidity predictions from: ${API_BASE_URL}/api/humidity/predict?day=${predictionDay}`);
      setDebugInfo(`Attempting to fetch from: ${API_BASE_URL}/api/humidity/predict?day=${predictionDay}`);
      
      const response = await fetch(`${API_BASE_URL}/api/humidity/predict?day=${predictionDay}`);
      ...
      console.log("Humidity prediction data received:", data);
      ...
      console.error("Error fetching humidity predictions:", err);
      setError(`Failed to load humidity predictions: ${err.message}`);
      setDebugInfo(prev => prev + `\nError: ${err.message}`);
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow duration-200 h-full flex items-center justify-center">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-gray-600">Loading humidity predictions...</p>
        </div>
      </div>
    );
  }
  ...
  if (!loading && !error && predictionData) {
    ...
    chartData = {
      ...
      datasets: [
        {
          label: `Day ${predictionData.day} Humidity Predictions`,
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow duration-200 h-full">
      <div className="mb-4 flex justify-between items-center">
        <div>
          <h2 className="text-xl font-semibold text-gray-800">Humidity Prediction</h2>
          <p className="text-sm font-medium text-gray-500">5-Day Hourly Humidity Forecast</p>
        </div>
      ...
      {error || !predictionData ? (
        <div className="flex flex-col items-center justify-center h-[300px]">
          ...
          <button 
            className="mt-4 px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg hover:from-blue-600 hover:to-blue-700 transition-colors duration-200 shadow-sm"
...
export default HumidityPrediction;
